...
def create_bear_researcher(llm):
    def bear_node(state) -> dict:
        ...
        extra_reports = ""
        if macro_report:
            extra_reports += f"\nMacro backdrop report: {macro_report}"
        if cn_sentiment_report:
            extra_reports += f"\nCN A-share sentiment report: {cn_sentiment_report}"
        if event_impact_report:
            extra_reports += f"\nEvent-driven impact report: {event_impact_report}"
        if capital_flow_report:
            extra_reports += f"\nCapital flow report (主力资金/北向/两融/龙虎榜): {capital_flow_report}"
